...
class Trainer():
  def __init__(self, ARCH, DATA, datadir, logdir, path=None):
    # parameters
    self.ARCH = ARCH #网络结构文件
    self.DATA = DATA  #data 是yml文件
    self.datadir = datadir  #datadir是数据集的文件夹
    self.log = logdir #日志文件夹
    self.path = path  #pretrain path
    ...
    # get the data settings nus lidar
    self.ARCH["dataset"]["sensor"]['fov_up'] =10.67
    self.ARCH["dataset"]["sensor"]['fov_down'] = -30.67
    self.ARCH["dataset"]["sensor"]['img_prop']['width'] =1024
    self.ARCH["dataset"]["sensor"]['img_prop']['height'] =32
  ...
  def train(self):
    ...
    # print(self.loss_w , "ignore" , self.ignore_class , "num ",self.parser.get_n_classes() )
    # print(self.log)
    # quit()
    # train for n epochs
    for epoch in range(self.ARCH["train"]["max_epochs"]):
      ...
      if epoch % 1 == 0:
        ...
        acc, iou, loss, rand_img = self.validate(val_loader=self.parser.get_valid_set(),
                                                 ...
                                                 save_scans=self.ARCH["train"]["save_scans"],
                                                 epoch = epoch)
        # np.save(self.log + "/{:04d}epoch_imgs".format(epoch), rand_img)
        # update info
        ...
        #   best_train_iou = iou
        #   self.model_single.save_checkpoint(self.log, suffix="_pre")
        #   quit()
  ...
  def validate(self, val_loader, model, criterion, evaluator, class_func, color_fn, save_scans , epoch=None):
    batch_time = AverageMeter()
    losses = AverageMeter()
    acc = AverageMeter()
    iou = AverageMeter()
    ...
    with torch.no_grad():
      ...
      tim=time.localtime()
      f=open( self.log+ "epoch{:04d}".format(epoch)+ ".txt","w")
      f.write(str(tim))
      f.write('\n')
      ...
      for i, jacc in enumerate(class_jaccard):
        ...
        f.write('IoU class {i:} [{class_str:}] = {jacc:.3f}'.format(
            i=i, class_str=class_func(i), jacc=jacc))
        f.write('\n')
      f.write('\n')
